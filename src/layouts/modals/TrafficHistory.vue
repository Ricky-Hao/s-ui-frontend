...
import { TrafficHistoryItem } from '@/types/autoreset'
...
export default {
  ...
  methods: {
    async loadData() {
      this.loading = true
      try {
        const result = await Data().loadTrafficHistory(this.clientId, this.page, this.pageSize)
        this.histories = result.histories
        this.total = result.total
        // Handle pagination edge case: page out of range but data exists
        if (this.histories.length === 0 && this.total > 0 && this.page > 1) {
